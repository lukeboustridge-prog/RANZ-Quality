...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app/api/admin/programme/[id]/route.ts"
      ...
      via: "sends PROGRAMME_STATUS_CHANGE notification on every status transition"
      pattern: "createNotification.*PROGRAMME_STATUS_CHANGE"
---
...
<task type="auto">
  ...
  <action>
    ...
    2. Create src/app/api/admin/programme/[id]/route.ts:
       ...
       - Accept JSON body with Zod validation:
         ...
         ```
       - Action handlers:
         - **approve**: Require current status is PENDING. Calculate anniversaryDate as exactly 1 year from now:
           ```typescript
           const now = new Date();
           const anniversaryDate = new Date(now);
           anniversaryDate.setFullYear(anniversaryDate.getFullYear() + 1);
           ```
           Set status=ACTIVE, activeSince=now, anniversaryDate=anniversaryDate (activeSince + 1 year), reviewedAt=now, reviewedBy=userId, reviewNotes=notes. Log ENROL_APPROVE audit event. Send PROGRAMME_STATUS_CHANGE notification to org via createNotification.
         - **reject**: Require current status is PENDING. Set status=WITHDRAWN, reviewedAt=now(), reviewedBy=userId, reviewNotes=notes. Log ENROL_REJECT audit event. Send PROGRAMME_STATUS_CHANGE notification to org via createNotification.
         - **suspend**: Require current status is ACTIVE or RENEWAL_DUE. Set status=SUSPENDED, suspendedAt=now(), suspendedBy=userId, suspendedReason=reason. Log ENROL_SUSPEND audit event. Send PROGRAMME_STATUS_CHANGE notification to org via createNotification.
         - **reinstate**: Require current status is SUSPENDED. Set status=ACTIVE, suspendedAt=null, suspendedBy=null, suspendedReason=null. Log ENROL_REINSTATE audit event. Send PROGRAMME_STATUS_CHANGE notification to org via createNotification.
       - For ALL notifications, use createNotification from @/lib/notifications with:
  ...
  <done>
    ...
    Notifications sent to org on status changes via createNotification.
...
<task type="auto">
  ...
  <done>
    ...
    Admin can approve PENDING applications (sets ACTIVE, activeSince, anniversaryDate=activeSince+1year).
...
- Approve sets anniversaryDate = activeSince + 1 year explicitly
