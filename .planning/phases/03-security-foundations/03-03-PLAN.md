...
must_haves:
  truths:
    ...
    - "Document creation (POST) is logged to AuditLog table"
  ...
  artifacts:
    ...
    - path: "src/app/api/documents/route.ts"
      provides: "Document POST with audit logging"
      contains: "logDocumentMutation"
    - path: "src/app/api/documents/[id]/approve/route.ts"
      provides: "Document approve/reject with audit logging"
      contains: "logDocumentMutation"
  ...
  key_links:
    ...
    - from: "src/app/api/documents/route.ts"
      to: "src/lib/audit-log.ts"
      via: "import logDocumentMutation"
      pattern: "import.*logDocumentMutation.*from.*audit-log"
...
<task type="auto">
  ...
  <files>src/app/api/documents/route.ts, src/app/api/documents/[id]/approve/route.ts, src/app/api/staff/route.ts, src/app/api/staff/[id]/route.ts, src/app/api/staff/[id]/verify-lbp/route.ts</files>
  <action>
1. Update src/app/api/documents/route.ts (POST handler):
   - Add import: `import { logDocumentMutation } from "@/lib/audit-log";`
   - After `createDocumentWithVersion()` returns successfully, add:
     ```typescript
...
2. Update src/app/api/documents/[id]/approve/route.ts:
   - Add import: `import { logDocumentMutation } from "@/lib/audit-log";`
...
3. Update src/app/api/staff/route.ts (POST handler):
...
4. Update src/app/api/staff/[id]/route.ts:
   - Add import: `import { logMemberMutation } from "@/lib/audit-log";`
...
5. Update src/app/api/staff/[id]/verify-lbp/route.ts:
   - Add import: `import { logMemberMutation } from "@/lib/audit-log";`
...
Grep for logDocumentMutation in documents routes (should find 3 usages: CREATE, APPROVE, REJECT).
Grep for logMemberMutation in staff routes.
  </verify>
  <done>
Document create/approve/reject and staff create/update/delete/verify all log to AuditLog.
  </done>
</task>
...
3. `grep -r "logDocumentMutation" src/app/api/documents/` shows 3 usages (CREATE, APPROVE, REJECT)
...
- Document creation (POST) logs to AuditLog
