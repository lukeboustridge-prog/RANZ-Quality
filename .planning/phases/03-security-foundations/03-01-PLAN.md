...
2. Create src/lib/cron-auth.ts:
  ...
  <verify>
1. Run `npx tsc --noEmit` - should pass (may show pre-existing errors but no new errors in env.ts or cron-auth.ts).
2. Check that cron-auth.ts exports verifyCronRequest function.
3. Test startup failure with missing CRON_SECRET:
   - Temporarily rename/remove CRON_SECRET from .env.local (or comment it out)
   - Run `npm run build` or `npm run dev`
   - Verify the build/startup fails with Zod validation error mentioning CRON_SECRET
   - Restore CRON_SECRET to .env.local
  </verify>
  <done>
env.ts requires CRON_SECRET with min 32 chars. cron-auth.ts exports verifyCronRequest() that returns 401 or null. App fails to start without CRON_SECRET.
  </done>
</task>
...
2. Update src/app/api/cron/notifications/route.ts:
  ...
  <verify>
1. Run `npx tsc --noEmit` - should pass for cron routes.
2. Grep both files to confirm they import verifyCronRequest from @/lib/cron-auth.
3. Grep to confirm no `process.env.CRON_SECRET` direct access remains in cron routes.
4. Test unauthorized cron logging:
   - Start dev server with `npm run dev`
   - Make unauthorized request: `curl -X GET http://localhost:3000/api/cron/verify-lbp`
   - Check server console output for logged warning containing: ip, userAgent, path, timestamp
  </verify>
  <done>
Both cron endpoints use verifyCronRequest() for authentication. No permissive fallback remains. Unauthorized requests return 401 and log warning with IP, userAgent, path, timestamp to server console.
  </done>
</task>
...
5. Startup failure verified: app fails to start without CRON_SECRET env var
6. Unauthorized logging verified: console.warn fires with IP, userAgent, path, timestamp on unauthorized cron request
</verification>
...
- Application fails to start if CRON_SECRET not set (Zod validation error)
- Unauthorized cron attempts are logged with IP, userAgent, path, timestamp
</success_criteria>
