...
Verify query filter includes nextRetryAt check:
```bash
grep -A10 "retryFailedNotifications" src/lib/notifications.ts | grep "nextRetryAt"
```
Expected: Should show `nextRetryAt: { lte: now }` or similar in the where clause.

Verify lastRetryAt is set before retry attempt:
```bash
grep -B2 -A2 "lastRetryAt" src/lib/notifications.ts
```
Expected: Should show `lastRetryAt: now` in an update call.

Verify nextRetryAt is calculated on failure:
```bash
grep "calculateNextRetryTime" src/lib/notifications.ts
```
Expected: Should show import and usage of calculateNextRetryTime function.
  </verify>
